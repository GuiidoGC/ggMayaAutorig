...
def make():
    ...
    basic_structure.create_basic_structure(asset_name="elephant")

    spm.SpineModule().make()
    nck.NeckModule().make()
    vfk.VariableFkModule(side="C", prefix="trunk").make()
    vfk.VariableFkModule(side="C", prefix="tail").make()
    

    for side in ["L", "R"]:
        lbm.FrontLegModule(side=side).make()
        lbm.BackLegModule(side=side).make()
